...
function quadro_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'quadro' ), max( $paged, $page ) );
 ...
 */
if ( ! function_exists( 'bop_breadcrumbs' ) ) :
function bop_breadcrumbs() {
	...
	if( is_404() ) echo __('Page not Found', 'quadro');
	...
	if( class_exists( 'Woocommerce' ) && is_woocommerce() && is_single() ) {
		...
		echo '<a href="' . get_permalink( $shop_id ) . '">' . get_the_title( $shop_id ) . '</a>';
		echo '</a> &raquo; ';
		the_title();
	}
 ...
 */
function quadro_widget_area( $display, $layout, $section_class, $area_class ){
	...
	if ( $quadro_options[$display] == 'show' ) { ?>
			...
			<div class="<?php echo $section_class; ?> clear">
				...
				if ( $display == 'widgetized_header_display' ) {
					echo '</div>';
					echo '<span id="header-handle" class="header-handler"></span>';
				} ?>
...
function quadro_excerpt( $excerpt = '', $excerpt_length = 20, $readmore = '', $tags = '' ) {
	...
	if ( count($string_check, COUNT_RECURSIVE) > $excerpt_length ) {
		...
		if ( $readmore != '' ) { 
			$short_content .= ' ... ' . '<a href="' . get_permalink($post->ID) . '" class="readmore-link">' . $readmore . '</a>';
		} else { 
			$short_content .= ' ... ';
		}
		echo $short_content;
		...
		echo $excerpt;
		if ( $readmore != '' ) { 
			echo '<a href="' . get_permalink($post->ID) . '" class="readmore-link">' . $readmore . '</a>';
		}

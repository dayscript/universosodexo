...
function qitheme_update_check($checked_data) {
...
function qitheme_api_call($def, $action, $args) {
	...
	if ( is_wp_error($request) ) {
		$res = new WP_Error('themes_api_failed', __('An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'quadro'), $request->get_error_message());
	} else {
		$res = unserialize($request['body']);
		if ($res === false)
			$res = new WP_Error('themes_api_failed', __('An unknown error occurred', 'quadro'), $request['body']);
	}
...
if ( isset($quadro_options['updates_enable']) && $quadro_options['updates_enable'] == true ) {
	add_filter('pre_set_site_transient_update_themes', 'qitheme_update_check');
	add_filter('themes_api', 'qitheme_api_call', 10, 3);

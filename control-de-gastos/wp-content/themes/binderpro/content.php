...
<?php // Bring correct thumbnail size
$thumb_size = '';
if ( $columns == 'two' || wp_is_mobile() ) {
	$thumb_size = 'med-thumb';
} elseif ( $columns == 'three' ) {
	$thumb_size = 'small-thumb';
}
...
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-item size-item'); ?>>
			...
			case 'gallery':
				if ( get_post_gallery() ) {
					...
					if ( isset($gallery['ids']) ) {
						...
						$content = quadro_strip_shortcode_gallery( get_the_content( '<span class="read-more">' . __('Leer Más', 'quadro') . '</span>' ) );
						...
						$content = get_the_content( '<span class="read-more">' . __('Leer Más', 'quadro') . '</span>' );
					}
				} else {
	...
	<?php // Display Excerpts for Search and Grid/Teaser Blog Styles
	if ( ($columns != 'three' || 'post' != $post_type || $post_format == 'quote' || $post_format == 'link' || $post_format == 'status' || $post_format == 'aside') ) :
	// ...but not for three columns blog.

	 	// We show here the excerpt, but not for these formats
	 	if ( ( $post_format == '' || $post_format == 'gallery' ) && $columns != 'three' ) { ?>
			<div class="entry-summary">
				<?php echo quadro_excerpt(get_the_excerpt(), 30, '<span class="read-more">' . __('Leer Más', 'quadro') . '</span>'); ?>
			</div><!-- .entry-summary -->
		<?php } else {
			if ( $post_format == 'quote' ) {
				...
				echo '</div>';
			} elseif ( ( $post_format == 'audio' || $post_format == 'video' ) && $columns != 'three' ) {
				echo '<div class="entry-summary">';
				echo quadro_excerpt(get_the_excerpt(), 30, '<span class="read-more">' . __('Leer Más', 'quadro') . '</span>');
				echo '</div>';
			} elseif ( $post_format == 'image' && $columns != 'three' ) {
				// nothing here
			} elseif ( $post_format == 'aside' || $post_format == 'status' ) {
				// Bring the full content for the other formats (aside & status between others)
				echo '<div class="entry-summary">';
				the_content( '<span class="read-more">' . __('Leer Más', 'quadro') . '</span>' );
				echo '</div>';
			} else { ?>
				<div class="entry-summary">
				<?php echo quadro_excerpt(get_the_excerpt(), 30, '<span class="read-more">' . __('Leer Más', 'quadro') . '</span>'); ?>
